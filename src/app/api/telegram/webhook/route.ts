...
/**
 * Кнопки постоянного меню (ADMIN_KEYBOARD) отправляют свой текст обычным
 * сообщением — здесь превращаем этот текст в тот же ответ, что дают
 * соответствующие /команды и инлайн-кнопки меню. Возвращает true, если
 * текст сообщения оказался одной из кнопок (и уже ответили), иначе false.
 */
async function handleKeyboardButton(chatId: string, text: string): Promise<boolean> {
  switch (text) {
    case "📅 Сегодня": {
    ...
    case "🕐 Свободные часы": {
      await sendMessage(chatId, await handleFreeCommand(parseDateArg("сегодня")!), withMenuButton());
      return true;
    }
    default:
      return false;
...
async function handleMessage(message: any) {
  ...
  // Нажатие на кнопку постоянного меню снизу приходит сюда обычным
  // текстовым сообщением (с тем же текстом, что написан на кнопке) — если
  // это одна из них, обрабатываем и не идём дальше в разбор /команд.
  if (await handleKeyboardButton(chatId, text.trim())) return;
  ...
  switch (cmd) {
    case "/menu":
      ...
      break;
    case "/keyboard":
      // На случай, если постоянное меню снизу куда-то пропало (например,
      // после переустановки Telegram) — присылает его заново.
      await sendMessageWithKeyboard(chatId, "⌨️ Меню снизу — обновлено.", ADMIN_KEYBOARD);
      break;
    case "/hidecommands":
      // Убирает старый список команд-подсказок, всплывающий по вводу "/"
      // (был настроен один раз через scripts/set-commands.ts) — теперь
      // вместо него постоянное меню снизу (кнопка "/keyboard" выше).
      await clearMyCommands();
      await sendMessage(chatId, "Готово — список команд по \"/\" больше не будет всплывать.");
      break;
    case "/today": {
      const reply = await handleDayCommand(parseDateArg("сегодня")!);
      await sendMessage(chatId, reply.text, withMenuButton(reply.buttons));
